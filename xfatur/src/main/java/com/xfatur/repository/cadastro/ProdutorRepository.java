...
public interface ProdutorRepository extends JpaRepository<Produtor, Integer> {
    ...
    @Query("select p.id as id, p.descricao as descricao from Produtor p where p.descricao like %:descricao%")
    Page<ProdutorView> findByDescricao(@Param("descricao") String descricao, Pageable pageable);
